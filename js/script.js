...
(function() {
    ...
    function connectWebSocket() {
        ...
        ws.onmessage = (event) => {
            ...
            try {
                ...
                console.warn('[CLIENT] WS RX: failed to parse message', event.data);
                return;
            }
            console.log('[CLIENT] WS RX', data);
    ...
    createRoomBtn.addEventListener('click', async () => {
        console.log('[CLIENT] createRoomBtn clicked', { userHandle, wsState: ws && ws.readyState });
        await ensureNotificationPermission();
        ...
        console.log('[CLIENT] WS TX', { type: 'CREATE_ROOM', handle: userHandle, roomName, opponentHandle, duration, interval, problemsCount: Array.isArray(problems) ? problems.length : 0 });
    });
    ...
    async function init() {
        ...
        function decorateStaticProfileLinks() {
            try {
                ...
                anchors.forEach(a => {
                    // Allow pages to opt-out of CF handle coloring by adding
                    // `data-no-cf-color` on the anchor (used in index footer)
                    if (a.hasAttribute('data-no-cf-color')) return;
                    try {
